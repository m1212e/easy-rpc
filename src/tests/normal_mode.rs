...
mod tests {
    ...
    #[tokio::test]
    async fn test_invalid_p() {
        match run_main(vec!["-p".to_string()]).await {
            Ok(_) => panic!("Should not be ok"),
            Err(err) => assert_eq!(err, "Could not find path argument after -p flag"),
        }
    }
    ...
    async fn test_success() {
        ...
        run_main(vec!["-p".to_string(), wdir.to_str().unwrap().to_string()]).await.unwrap();
