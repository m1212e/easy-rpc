...
mod tests {
    use crate::transpiler::parser::{
        ...
        lexer::invalid_characters::InvalidCharacters, CodeArea,
    };
    ...
    fn test_success() -> Result<(), InputReaderError> {
        ...
        assert_eq!(output.is_some(), true);
        let output = output.unwrap();
        assert_eq!(output.get_start().character, 1);
        assert_eq!(output.get_start().line, 0);
        ...
        assert_eq!(output.is_some(), true);
        let output = output.unwrap();
        assert_eq!(output.get_start().character, 2);
        assert_eq!(output.get_start().line, 0);
        ...
        assert_eq!(output.is_some(), true);
        let output = output.unwrap();
        assert_eq!(output.get_start().character, 3);
        assert_eq!(output.get_start().line, 0);
